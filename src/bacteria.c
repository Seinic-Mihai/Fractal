...
/*   Updated: 2016/04/13 17:11:22 by mseinic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <fractol.h>

static void				bacteria_iter(t_fract *fract, size_t maxiter)
{
	t_cmplx	tmp;
...
static void				create_bacteria(t_env *env)
{
	t_fract		fract;
	...
	while (fract.y < WIN_H)
	{
		...
		while (fract.x < WIN_W)
		{
			...
			coordonates(&fract);
			bacteria_iter(&fract, env->maxiter);
